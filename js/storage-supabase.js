...
function mapRaceDbToLocal(r) {
  return {
    ...
    // Épreuve ouverte au VAE (nouvelle UI). Compat avec ancien champ "ebike".
    ebikeOpen: !!(r.ebike_open ?? r.ebike),
    ebike: !!(r.ebike_open ?? r.ebike),
    ebikeStartTime: r.ebike_start_time ?? null,
...
function mapRaceLocalToDb(ev, organizerId) {
  return {
    ...
    // On accepte ev.ebikeOpen (nouvelle UI) ou ev.ebike (legacy)
    ebike_open: !!(ev.ebikeOpen ?? ev.ebike),
    ebike: !!(ev.ebikeOpen ?? ev.ebike),
    ebike_start_time: ev.ebikeStartTime ?? null,
